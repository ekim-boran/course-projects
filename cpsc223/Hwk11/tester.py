...
class Tester:
    ...
    def __init__(self, dueDate = None, latePenalty = None, gracePeriod = 86400, latenessRate = None, dir = '.'):
        ...
        self._startTime = time.time() # for reporting
        # set up playground
    ...
    def report(self):
        ...
        return '\n'.join([self.reportHeader.format(self.points, self.maxpoints, self.latePenalty, 100.0*self.lateness, finalPoints, self.maxpoints)] 
                + self._report
                + ["Total time: {:.2f} seconds".format(duration)])
