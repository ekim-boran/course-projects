...
TEST_F(ExecutorTest, DISABLED_SimpleRawInsertWithIndexTest) {
  ...
  auto out_schema = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
  SeqScanPlanNode scan_plan{out_schema, nullptr, table_info->oid_};
  SeqScanPlanNode scan_plan2 = scan_plan;
  ...
  // Verify
  // std::cout << "Index with predicate" << std::endl;
  for (const auto &tuple : result_set) {
  ...
  IndexScanPlanNode plan1{out_schema, nullptr, index_info->index_oid_};

  // Execute
  GetExecutionEngine()->Execute(&plan1, &result_set, GetTxn(), GetExecutorContext());
  // Verify
  std::cout << std::endl << "Full index scan" << std::endl;
  // for (const auto &tuple : result_set) {
  //  //std::cout << tuple.GetValue(out_schema, out_schema->GetColIdx("colA")).GetAs<int32_t>() << ","
  //  //          << tuple.GetValue(out_schema, out_schema->GetColIdx("colB")).GetAs<int32_t>() << " -- ";
  //}

  IndexScanPlanNode plan2{out_schema, nullptr, index_info->index_oid_};

  std::unordered_map<uint32_t, UpdateInfo> update_attrs = {{0, UpdateInfo{UpdateType::Add, 10}}};
  UpdatePlanNode update_plan{&plan2, table_info->oid_, update_attrs};
  result_set.clear();
  GetExecutionEngine()->Execute(&update_plan, &result_set, GetTxn(), GetExecutorContext());
  // std::cout << "After Update" << std::endl;
  result_set.clear();
  IndexScanPlanNode plan3{out_schema, nullptr, index_info->index_oid_};
  GetExecutionEngine()->Execute(&plan3, &result_set, GetTxn(), GetExecutorContext());
  // for (const auto &tuple : result_set) {
  //  std::cout << tuple.GetValue(out_schema, out_schema->GetColIdx("colA")).GetAs<int32_t>() << ","
  //            << tuple.GetValue(out_schema, out_schema->GetColIdx("colB")).GetAs<int32_t>() << " -- " << std::endl;
  //}
...
TEST_F(ExecutorTest, SchemaChangeSeqScan) {
  ...
  std::unique_ptr<AbstractPlanNode> scan_plan1;
  std::unique_ptr<AbstractPlanNode> scan_plan3;
  ...
  const Schema *out_schema1;
  {
    ...
    out_schema1 = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
    scan_plan1 = std::make_unique<SeqScanPlanNode>(out_schema1, predicate, table_info->oid_);
    scan_plan3 = std::make_unique<SeqScanPlanNode>(out_schema1, predicate, table_info->oid_);
  }
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(scan_plan3.get(), &result_set, GetTxn(), GetExecutorContext());
  result_set.clear();
...
TEST_F(ExecutorTest, SimpleIndexScanTest) {
  ...
  auto *colA = MakeColumnValueExpression(schema, 0, "colA");
  auto *colB = MakeColumnValueExpression(schema, 0, "colB");
  auto *const600 = MakeConstantValueExpression(ValueFactory::GetIntegerValue(998));
  auto *predicate = MakeComparisonExpression(colA, const600, ComparisonType::GreaterThan);
  auto *out_schema = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
