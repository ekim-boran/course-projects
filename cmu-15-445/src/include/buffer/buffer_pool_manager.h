...
class BufferPoolManager {
 public:
  ...
  Page *FetchPage(page_id_t page_id, bufferpool_callback_fn callback = nullptr) {
    // LOG_INFO("fetch page id %d", page_id);
 ...
 protected:
  ...
  Page *NewPageImpl1(page_id_t *page_id, int index);
  ...
  bool GetPageTable(page_id_t page_id, frame_id_t *frame_id);
  bool GetFreeList(frame_id_t *frame_id, int index);

  void WritePage(page_id_t page_id, const char *page_data);

  void ReadPage(page_id_t page_id, char *page_data);
  int AllocatePage(int index);
  ...
  Page *pages_[NUMPOOLS];

  /** Page table for keeping track of buffer pool pages. */
  std::unordered_map<page_id_t, frame_id_t> page_table_[NUMPOOLS];
  /** Replacer to find unpinned pages for replacement. */
  Replacer *replacer_[NUMPOOLS];
  /** List of free pages. */
  std::list<frame_id_t> free_list_[NUMPOOLS];
  /** This latch protects shared data structures. We recommend updating this comment to describe what it protects. */
  SpinLock latch_[NUMPOOLS];
  std::mutex db_io_latch_;
  std::atomic<u_int32_t> start_index_ = 0;
  std::atomic<int> last_ids[NUMPOOLS];
