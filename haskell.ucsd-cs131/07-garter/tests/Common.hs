...
import Language.Garter.Parser
import Language.Garter.Normalizer
...
run name heapMb pgm = do
  ...
  where
    ...
    cmd    = printf "rm -f %s && make %s %s" resF resF heapSz
...
createTestTree sc Test{..} = let res = case testResult of
                                         Value   s -> Right s
                                         Failure s -> Left  s
                                 mk = if testAnf then anfTest else mkTest (heapSize testHeap)
...
------------------------------------------------------------------------------------
------------------------------------------------------------------------------------
gatherTests ok dir sc = do
  files <- getFiles ok dir
  testGroup "Tests" <$> mapM (gatherTest sc) files

gatherTest sc testFile  = do
  tests   <- readTests sc testFile
  let name = mkTestName testFile
  return (testGroup name tests)

mkTestName :: String -> String
mkTestName f = case splitOn "___" (takeFileName f) of
                 [name] -> dropSuffix ".json" name
                 name:_ -> name

dropSuffix suff s 
  | L.isSuffixOf suff s = take (length s - length suff) s
  | otherwise           = s 

readTests     :: Score -> FilePath -> IO [TestTree]
readTests sc f = map (createTestTree sc) <$> parseTestFile f

getFiles :: (FilePath -> Bool) -> FilePath -> IO [FilePath]
getFiles ok path = do 
  files  <- map (path </>) <$> Dir.listDirectory path 
  files' <- filterM Dir.doesFileExist files
  return (filter ok (L.sort files')) 
