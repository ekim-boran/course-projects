...
wellFormedD fEnv (Decl _ xs e _) =
  duplicateParamErrors xs ++ wellFormedE fEnv vEnv e
  where vEnv = addsEnv xs emptyEnv
...
wellFormedE fEnv = go
 where
  ...
  go vEnv (App f es l) =
    callArityErrors fEnv f es l ++ unboundFunErrors fEnv f l ++ gos vEnv es
...
duplicateFunErrors ds = map errDupFun . concat . dupBy (bindId . fName) $ ds

duplicateParamErrors :: [BareBind] -> [UserError]
duplicateParamErrors xs = map errDupParam . map head . dupBy bindId $ xs
...
callArityErrors fEnv f es l = case lookupEnv f fEnv of
