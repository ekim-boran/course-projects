...
/*******************************************************************************
  ...
  Arguments:
    ...
    - stack_bottom: A pointer to the bottom of the stack (highest address).  
      In our runtime, this is set by getting the address of a variable from 
      main, which itself is assigned by the assembly preamble.

    - stack_top: A pointer to the top of the stack (lowest address).  During
      execution, this will be the current value of RSP, so it points to the
  ...
  Returns:
    ...
    allocation index stored in R15.
