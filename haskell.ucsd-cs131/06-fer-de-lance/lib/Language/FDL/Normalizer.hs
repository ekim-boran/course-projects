...
module Language.FDL.Normalizer
  ( anormal
  ) where
...
anf i (Tuple es l) = (i', stitch bs (Tuple es' l))
  where (i', bs, es') = imms i es

anf i (GetItem e1 e2 l) = (i', stitch bs (GetItem e1' e2' l))
  where (i', bs, [e1', e2']) = imms i [e1, e2]


anf i (App e es l) = (i', stitch bs (App e' es' l))
  where (i', bs, e' : es') = imms i (e : es)

anf i (Lam xs e l  ) = (i', Lam xs e' l) where (i', e') = anf i e

anf i (Fun f xs e l) = (i', Fun f xs e' l) where (i', e') = anf i e
...
--------------------------------------------------------------------------------
fresh l i = (i + 1, Bind x l) where x = "anf" ++ show i
