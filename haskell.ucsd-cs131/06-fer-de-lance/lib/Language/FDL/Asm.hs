...
--------------------------------------------------------------------------------
asm instrs = header <> instrsAsm (prelude ++ instrs ++ postlude) <> "\n"
...
postlude = concat
  [ dynError (TypeError TNumber)  0
  , dynError (TypeError TBoolean) 1
  , dynError ArithOverflow        2
  , dynError (TypeError TTuple)   3
  , dynError IndexLow             4
  , dynError IndexHigh            5
  , dynError (TypeError TClosure) 6
  , dynError ArityError           7
  ]

dynError :: DynError -> Int -> [Instruction]
dynError e n =
  [ ILabel (DynamicErr e)
  , IMov (Reg RDI) (Const n)
  , ICall (CodePtr $ Builtin "error")
  ]
