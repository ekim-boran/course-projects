...
defExpr :: Parser Bare
defExpr = defs <$> some funDefExpr <*> expr

defs :: [Bare] -> Bare -> Bare
defs []     e   = e
defs (d:ds) e   = Let f d e' (stretch [d, e'])
  where 
    e'          = defs ds e
    Fun f _ _ _ = d

funDefExpr :: Parser Bare 
funDefExpr = withSpan' $ do
  rWord "def"
  f  <- binder
  xs <- parens (sepBy binder comma) <* colon
  -- _  <- try (rWord "in") <|> blank
  Fun f xs <$> expr

-- blank :: Parser SourceSpan
-- blank = snd <$> withSpan (pure ())
