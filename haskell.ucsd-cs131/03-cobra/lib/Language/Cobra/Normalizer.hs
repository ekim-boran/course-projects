...
module Language.Cobra.Normalizer
  ( anormal
  ) where
...
anf i (Let x e b l) = (i'', Let x e1' e2' l) where
  (i' , e1') = anf i e
  (i'', e2') = anf i' b
...
--------------------------------------------------------------------------------
imm i num@( Number  _ _ ) = (i, [], num)

imm i id@(  Id      _ _ ) = (i, [], id)

imm i bool@(Boolean b l ) = (i, [], bool)
...
imm i (Prim2 o e1 e2 l) = (i'', bs, mkId v l)
 where
  (i', b1s, [v1, v2]) = imms i [e1, e2]
  (i'', v)            = fresh l i'
  bs                  = (v, (Prim2 o v1 v2 l, l)) : b1s
