...
--------------------------------------------------------------------------------
scoreTest'
  :: (Show b, Eq b) => Score -> (a -> b, a, b, Int, String) -> TestTree
--------------------------------------------------------------------------------
scoreTest' sc (f, x, expR, points, name) = testCase name $ do
  updateTotal sc points
  if f x == expR then updateCurrent sc points else assertFailure "Wrong Result"

-- check :: Result -> Result -> TestTree
check sc n (Right resV) (Right expectV)
  | trim expectV == trim resV = updateCurrent sc n
  | otherwise                 = assertFailure resV
check sc n (Left resE) (Left expectE)
  | matchError expectE resE = updateCurrent sc n
  | otherwise               = assertFailure "Wrong error"

check _ _ (Left resE) (Right expectV) =
  assertEqual "Unexpected error" ("Value " ++ expectV) ("Error " ++ resE)
check _ _ (Right resV) (Left expectE) =
  assertEqual "Unexpected result" ("Error " ++ expectE) ("Value " ++ resV)

matchError expectE resE = tx expectE `isInfixOf` tx resE
  where tx = map toLower
--------------------------------------------------------------------------------
...
instance FromJSON TestResult where
  parseJSON = withObject expected $ \obj -> helper (M.toList obj) obj
   where
    ...
    helper [("value", val)] _ =
      withText "String" (return . Value . T.unpack) val
    helper [("failure", err)] _ =
      withText "String" (return . Failure . T.unpack) err
    helper _ obj = typeMismatch expected (Object obj)

instance FromJSON Test where
  parseJSON = withObject expected p
   where
    p o = if length (M.keys o) == 3 || length (M.keys o) == 4
      then
        Test
        <$> o
        .:  "name"
