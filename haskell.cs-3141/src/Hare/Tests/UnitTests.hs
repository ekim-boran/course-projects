...
module Hare.Tests.UnitTests where
...
test_empty_maybe =
  testProperty "empty (maybe)" $ \s -> (s =~ Empty) == Just ()

test_empty_list = testProperty "empty (list)" $
  \s -> (s =~ Empty) == replicate (length s + 1) ()

test_fail_maybe =
  testProperty "fail (maybe)" $ \s -> (s =~ Fail) == (Nothing :: Maybe ())

test_fail_list = testProperty "fail (list)" $ \s -> (s =~ Fail) == ([] :: [()])
...
test_seq_5 = testProperty "two chars in seq" $ \s c1 c2 ->
  case (s =~ (Char c1 `Seq` Char c2)) of
...
test_ch_3 = testProperty "two chars in choose" $ \s c1 c2 ->
  ((s =~ (Char c1 `Choose` Char c2)) :: Maybe Char) == (s =~ Char (c1 ++ c2))

test_ch_4 = testProperty "choose and seq 1" $ \s c1 c2 c3 ->
  ((s =~ (Char c1 `Seq` (Char c2 `Choose` Char c3))) :: [(Char, Char)])
    == ((s =~ ((Char c1 `Seq` Char c2) `Choose` (Char c1 `Seq` Char c3))))

test_ch_5 = testProperty "choose and seq 2" $ \s c1 c2 c3 ->
  ((s =~ ((Char c2 `Choose` Char c3) `Seq` Char c1)) :: [(Char, Char)])
    == ((s =~ ((Char c2 `Seq` Char c1) `Choose` (Char c3 `Seq` Char c1))))

test_star_1 = testProperty "star replicate" $ \n c ->
  let s = replicate n c
   in sort (s =~ Star (Char [c])) == sort (concatMap inits (inits s))

test_star_2 = testProperty "star fail" $
  \s -> ((s =~ Star Fail) :: [[()]]) == replicate (length s + 1) []

test_star_3 = testProperty "star char" $ \s c ->
  (all (== []) ((s =~ Star (Char c)) :: [String])) == not (any (`elem` c) s)
