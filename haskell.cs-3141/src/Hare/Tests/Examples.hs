module Hare.Tests.Examples where
...
test_action_ex_1 =
  testCase "action example 1" $
    assertEq
      ("***" =~ Action length (Star (Char ['*'])))
      [3, 2, 1, 0, 2, 1, 0, 1, 0, 0]

test_action_ex_2 =
  testCase "action example 2" $
    assertEq ("AXax" =~ Action isUpper (Char ['a', 'A'])) [True, False]
...
test_cons_ex_1 = testCase "cons example 1" $ do
  assertEq
    ("10100" =~ cons (Char ['1']) (Star (Char ['0'])))
    ["10", "1", "100", "10", "1"]

test_cons_ex_2 = testCase "cons example 2" $ do
  assertEq ("10100" =~ cons (Char ['1']) (Action (const []) Empty)) ["1", "1"]

test_plus_ex_1 = testCase "plus example 1" $ do
  assertEq ("10100" =~ plus (Char ['0'])) ["0", "00", "0", "0"]

test_plus_ex_2 = testCase "plus example 2" $ do
  let atoz = Char ['a' .. 'z']
  let digits = Char ['0' .. '9']
  assertEq
    ("ab1c3" =~ plus (atoz `Seq` digits))
    [[('b', '1'), ('c', '3')], [('b', '1')], [('c', '3')]]
...
test_choose_ex_1 = testCase "choose example 1" $ do
  ...
  assertEq
    ("COMP3141, MATH1081, PHYS1121, COMP3121" =~ re)
    ["COMP", "MATH", "PHYS", "COMP"]
...
test_option_ex_1 = testCase "option example 1" $ do
  ...
  assertEq
    ("-30 5 3" =~ (sign `cons` plus digits))
    ["-30", "-3", "+30", "+3", "+0", "+5", "+3"]

test_option_ex_2 = testCase "option example 2" $ do
  assertEq ("foo" =~ option (Char ['a'])) [Nothing, Nothing, Nothing, Nothing]
