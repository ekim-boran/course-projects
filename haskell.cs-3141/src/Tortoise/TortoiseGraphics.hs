module Tortoise.TortoiseGraphics (drawPicture, writePng) where

-- Rasterific

import Codec.Picture
import Graphics.Rasterific hiding (Line, Path, Point, Vector)
import Graphics.Rasterific.Texture
import Tortoise.Tortoise (Colour (..), Line (..), LineStyle (..), Picture)
  ...
  where
    ...
    drawLine (Line lineStyle colour (x1, y1) (x2, y2)) =
      texture colour $
        style lineStyle $
          polyline
            [ V2 (fromIntegral x1 + 400) (400 - fromIntegral y1),
