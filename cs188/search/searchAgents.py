...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, 15)
    ...
    def getSuccessors(self, state):
        ((pos_x, pos_y), val) = state
        successors = []
        actions = [Directions.NORTH, Directions.SOUTH,
                   Directions.EAST, Directions.WEST]
        coordinates = [(0, 1), (0, -1), (1, 0), (-1, 0)]
        for (action, (x, y)) in zip(actions, coordinates):
            newpos = (pos_x + x, pos_y + y)
            if(self.walls[newpos[0]][newpos[1]]):
                continue
            if newpos in self.corners:
                index = self.corners.index(newpos)
                successors.append(((newpos, val & ~(1 << index)), action, 1))
            else:
                successors.append(((newpos, val), action, 1))
        self._expanded += 1
        # print(successors)
        return successors
...
def manhattan(pos, corner):
    return (abs(pos[0] - corner[0]) + abs(pos[1] - corner[1]))


def cost_of_path(corners, pos):
    sum = 0
    for c in corners:
        sum += manhattan(pos, c)
        pos = c
...
def cornersHeuristic(state, problem):
    ...
    # print(dots, unvisited_corners )

    min = 99999999
    for p in permutations(unvisited_corners):
        c = cost_of_path(p, pos)
        if(min > c):
            min = c
    return min
...
def foodHeuristic(state, problem):
    if('paths' not in problem.heuristicInfo):
        problem.heuristicInfo['paths'] = dict()

    position, foodGrid = state
    max = 0
    for food in foodGrid.asList():
        if((position, food) not in problem.heuristicInfo['paths']):
            p = PositionSearchProblem(
                problem.startingGameState, goal=food, start=position, warn=False, visualize=False)
            problem.heuristicInfo['paths'][(
                position, food)] = len(search.bfs(p))
        d = problem.heuristicInfo['paths'][(position, food)]
