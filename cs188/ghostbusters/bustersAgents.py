...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        mindistance = 100
        loc = (0, 0)
        for gbeliefs in livingGhostPositionDistributions:     
            l, belief = max(gbeliefs.items(), key=lambda a: a[1])
            olddistance = self.distancer.getDistance(l, pacmanPosition)
            if olddistance < mindistance:
                mindistance = olddistance
                loc = l
        # bug if none of the actions makes the distance shorter it return None
        action = None
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            olddistance = self.distancer.getDistance(successorPosition, loc)
            if olddistance < mindistance:
                mindistance = olddistance
                action = a 
        #print(action)
